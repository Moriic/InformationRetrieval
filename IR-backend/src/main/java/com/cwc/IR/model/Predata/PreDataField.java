...
@Data
@Slf4j
public class PreDataField implements PreData {
    private static PreDataField instance = new PreDataField();
...
//    Map<String, Map<Integer, ArrayList<Integer>>> positionalIndex = new TreeMap<>();
    ...
    private PreDataField() {
        Init();
        log.info("研究领域数据初始化完毕");
    }

    // 编译正则表达式
    Pattern pattern = Pattern.compile("<研究领域>(.*?)</研究领域>",  Pattern.DOTALL);
    ...
    public static PreDataField newInstance() {
        return instance;
    }
}
